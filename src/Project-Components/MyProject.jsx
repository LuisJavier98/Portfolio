...
const MyProject = ({ language, ProjectMove, ProjectNext, SelectCard, projectNumber, ElementS, dark }) => {
  return (
    <div className={dark ? 'card_projects' : 'card_projectsLight'} id='projects'>
      ...
      <div className='text_project' data-aos="zoom-in-up" style={{ textAlign: 'center', color: dark ? 'white' : 'black' }}><h2>{language ? 'Projects' : 'Proyectos'}</h2><hr /> <p style={{ color: dark ? 'white' : 'black' }}>{language ? 'These are 8 of my projects front end developed personality' : 'Estos son 8 de mis proyectos desarrollados personalmente'}
      </p>
      </div>
    </div>
  )
